...
| AI model | Claude (`ANTHROPIC_MODEL`) | single dependency covers OCR, captioning, translation, classification, extraction; strong structured-output support. |
...
- Strict JSON: assistant reply is prefilled with `{`; output is schema-validated with Pydantic; one retry on invalid JSON, then the step fails cleanly and the raw output is logged to `audit_event`.
