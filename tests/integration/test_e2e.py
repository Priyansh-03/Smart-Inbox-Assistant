...
SAMPLES = os.getenv("SAMPLE_DIR", "sample-data")


def _import(api: str, *eml_names: str) -> str:
    files = [("files", (n, open(f"{SAMPLES}/emails/{n}", "rb"), "message/rfc822")) for n in eml_names]
...
def test_ec1_duplicate_message_id_is_noop(api):
    ...
    files = [("files", ("email_icsr_full.eml", open(f"{SAMPLES}/emails/email_icsr_full.eml", "rb"), "message/rfc822"))]
